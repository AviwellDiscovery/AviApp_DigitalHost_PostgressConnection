...
# =============================================================================
# Nk Network Multiomic - Monotone Correlation Graph Visualization
# =============================================================================
from django.http import JsonResponse
from .nk_network.router import table_for
from .nk_network.repo import fetch_seed_candidates
from .nk_network.graph_engine import build_graph, BuildParams
from .nk_network.viz import to_cytoscape_elements, CYTOSCAPE_STYLESHEET, CYTOSCAPE_LAYOUTS
from .nk_network.config import TISSUES, TISSUE_LABELS, DEFAULT_TOPK, DEFAULT_KN, DEFAULT_ABS_THR, DEFAULT_ACC_MIN, DEFAULT_CORR_MIN, DEFAULT_CORR_MAX


@csrf_exempt
def nk_network_load_seeds(request):
    """
    AJAX endpoint to load seed candidates for a given tissue.
    """
    ...
    try:
        seeds = fetch_seed_candidates(table, limit=limit)
        return JsonResponse({
            'seeds': seeds,
            'tissue': tissue,
            'count': len(seeds)
        ...
        import logging
        logging.error(f"Error loading seeds: {e}")
        return JsonResponse({'error': str(e)}, status=500)


@csrf_exempt
def nk_network_build_graph(request):
    """
    AJAX endpoint to build the monotone correlation graph.
    Returns Cytoscape-compatible JSON elements.
    """
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        seed_tissue = request.POST.get('seed_tissue', 'ileum')
        seed_id = request.POST.get('seed_id', '')

        # Get allowed tissues (multi-select)
        allowed_tissues = request.POST.getlist('allowed_tissues[]')
        if not allowed_tissues:
            allowed_tissues = TISSUES

        # Get numeric parameters
        top_k = int(request.POST.get('top_k', DEFAULT_TOPK))
        kn = int(request.POST.get('kn', DEFAULT_KN))
        acc_min = float(request.POST.get('acc_min', DEFAULT_ACC_MIN))
        abs_thr = float(request.POST.get('abs_thr', DEFAULT_ABS_THR))
        corr_min = float(request.POST.get('corr_min', DEFAULT_CORR_MIN))
        corr_max = float(request.POST.get('corr_max', DEFAULT_CORR_MAX))

        # Get visualization options
        layout_name = request.POST.get('layout', 'cose')
        highlight_node = request.POST.get('highlight', None)
        focus_mode = request.POST.get('focus_mode', 'off') == 'on'

        if not seed_id:
            return JsonResponse({'error': 'Seed ID is required'}, status=400)

        # Build the graph
        seed = (seed_tissue, seed_id)
        params = BuildParams(
            kn=kn,
            top_k=top_k,
            acc_min=acc_min,
            ...
            corr_min=corr_min,
            corr_max=corr_max,
            allowed_to_tissues=allowed_tissues
        )

        df_edges, layers = build_graph(seed, params)
        ...
        # Apply focus mode if requested
        if focus_mode and highlight_node:
            from .nk_network.viz import apply_focus
            elements = apply_focus(elements, highlight_node)

        # Get layout config
        layout_config = CYTOSCAPE_LAYOUTS.get(layout_name, CYTOSCAPE_LAYOUTS['cose'])
        ...
        return JsonResponse({
            ...
            'stats': {
                'nodes': len([e for e in elements if 'source' not in e.get('data', {})]),
                'edges': len([e for e in elements if 'source' in e.get('data', {})]),
                'seed_tissue': seed_tissue,
                'seed_id': seed_id,
                'depth': kn,
                'top_k': top_k
            }
